...
const getOSRMRouteDistance = async (pickup, drop) => {
  try {
    ...
    // Nominatim requires 1 request per second max, parallel requests can cause long hangs or IP blocks.
    const originResult = await fetchWithTimeout(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(pickup)}&format=json&limit=1`, 2500);
    const destResult = await fetchWithTimeout(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(drop)}&format=json&limit=1`, 2500);
    ...
    if (originResult && originResult.length > 0 && destResult && destResult.length > 0) {
      ...
      const osrmTimeoutId = setTimeout(() => osrmController.abort(), 5000);
      
      const osrmResult = await fetch(`https://router.project-osrm.org/route/v1/driving/${originLon},${originLat};${destLon},${destLat}?overview=false`, { signal: osrmController.signal }).then(r => r.json());
...
export const getRouteDistance = async (pickup, drop) => {
  ...
  // 4. Absolute Fallback (Demo/Testing mode)
  console.warn("All route calculations failed. Returning default fallback distance to prevent UI block.");
  const fallbackResult = {
    ...
    tollCount: 2,
    source: 'fallback',
    distanceSource: 'Estimated (Fallback)',
    isUnknownRoute: true
  };
